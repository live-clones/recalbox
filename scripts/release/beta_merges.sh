...
for issue in "${issuesArray[@]}";do
    ...
    tagmessage "- **Issue** [${issue_ref}](${issue_link}) ${issue_assignee:+"assigned to @${issue_assignee}"}"
    ...
    for mr in "${mrArray[@]}";do
        ...
        tagmessage "  - **Merge request** [${mr_ref}](${mr_link}) ${mr_assignee:+"assigneed to @${mr_assignee}"}"
...
if [[ "${MERGE}" == 1 ]];then
    ...
    if [[ "${PUSH}" == "1" ]];then
        ...
        cd "${CURDIR}/recalbox"
	tagmessage "### TESTING.md\n"
	git diff master...HEAD --no-ext-diff --unified=0 --exit-code -a --no-prefix -- TESTING.md | egrep "^\+-" | sed "s/^+//" >> "${CURDIR}/.tag_message"
        cd "${CURDIR}"
        if [[ "${TAG}" != "" ]];then
            # Create tag
            ...
            -d "description=$(cat "${CURDIR}/.tag_message")" \
            -d "name=${TAG}" \
            -d "tag_name=${TAG}" \
