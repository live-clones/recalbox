...
class ConfigGenerator:
    ...
    def loadConfigFile(self, filename: str):
        ...
        self.settings.loadFile(False).defineBool('true', 'false')
    ...
    def SetDefaultPath(self, subsystem: SubSystems):
        ...
        self.settings.setString("config_version", "4.4.0")
    ...
    def SetJoystick(self, subsystem: SubSystems, controllers: ControllerPerPlayer):
        ...
        if subsystem in SubSystems.HAVEMOUSE:
            ...
            self.settings.setString("joyport0_autofire", "none")
            self.settings.setString("joyport0_mode", "mousenowheel")
            self.settings.setString("joyport0_mousemap", "right")
            self.settings.setString("joyport0_friendlyname", "Mouse")
            self.settings.setString("joyport0_name", "MOUSE0")
            # Set joysticks
            for key in controllers:
                controller = controllers[key]
                player = int(controller.PlayerIndex)
                indexPadSDL = int(controller.SdlIndex) + 1
                if 1 <= player <= 3:
                    self.settings.setString("joyport{}".format(key), "joy{}".format(indexPadSDL))
                    self.settings.setString("joyport{}_autofire".format(key), "none")
                    self.settings.setString("joyport{}_mode".format(key), "cd32joy" if subsystem == SubSystems.CD32 else "djoy")
    ...
    def SetGraphics(self, subsystem: SubSystems):
        if subsystem == SubSystems.A600:
            ...
            self.settings.setInt("gfx_height", 256)
            ...
            self.settings.setBool("gfx_fullscreen_amiga", False)
            self.settings.setBool("gfx_fullscreen_picasso", False)
            ...
            self.settings.setInt("gfx_height", 270)
            self.settings.setInt("gfx_refreshrate", 50)
            self.settings.setInt("gfx_refreshrate_rtg", 50)
            self.settings.setBool("gfx_lores", False)
            self.settings.setString("gfx_resolution", "hires")
