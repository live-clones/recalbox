...
def defineControllerKeys(controller):
    ...
    for inputIdx in controller.inputs:
        inp = controller.inputs[inputIdx]
        if inp.name in mupenmapping and mupenmapping[inp.name] != "":
            value = setControllerLine(mupenmapping, inp, mupenmapping[inp.name])
            # Handle multiple inputs for a single N64 Pad inp
            if mupenmapping[inp.name] not in config:
                config[mupenmapping[inp.name]] = value
            else:
                config[mupenmapping[inp.name]] += " " + value
...
def setControllerLine(_, inp, mupenSettingName):
    ...
    inputType = inp.type
    if inputType == 'button':
        value = "button({})".format(inp.id)
    elif inputType == 'hat':
        value = "hat({} {})".format(inp.id, mupenHatToAxis[inp.value])
    elif inputType == 'axis':
        ...
        if mupenSettingName in mupenDoubleAxis.values():
            ...
            if inp.value == "-1":
                value = "axis({}-,{}+)".format(inp.id, inp.id)
            else:
                value = "axis({}+,{}-)".format(inp.id, inp.id)
        else:
            if inp.value == "1":
                value = "axis({}+)".format(inp.id)
            else:
                value = "axis({}-)".format(inp.id)
    return value
