...
RG353XPowerEventReader::RG353XPowerEventReader(HardwareMessageSender& messageSender)
...
RG353XPowerEventReader::~RG353XPowerEventReader()
{
  StopReader();
}

void RG353XPowerEventReader::StartReader()
{
  { LOG(LogDebug) << "[RG353X] Power button manager requested to start."; }
  Start("OAG2Power");
}

void RG353XPowerEventReader::StopReader()
{
  { LOG(LogDebug) << "[RG353X] Power button manager requested to stop."; }
  Stop();
}

void RG353XPowerEventReader::Break()
{
  if (mFileHandle >= 0)
  {
    { LOG(LogDebug) << "[RG353X] Breaking power button thread."; }
    mFileHandle = -1;
  }
}

void RG353XPowerEventReader::Run()
{
  { LOG(LogInfo) << "[RG353X] Running background power button manager."; }
  while(IsRunning())
  {
    ...
    while(IsRunning())
    {
      ...
      if (read(mFileHandle, &event, sizeof(event)) != sizeof(event))
      {
        ...
        { LOG(LogInfo) << "[RG353X] Power event reader ordered to stop."; }
        break;
      }
...
void RG353XPowerEventReader::Suspend()
{
  { LOG(LogInfo) << "[RG353X] SUSPEND!"; }
  mWaitFor = WaitFor::Ignore; // Ignore next event when waking up!
  if (system("/usr/sbin/pm-suspend") != 0) // Suspend mode
  { LOG(LogError) << "[RG353X] Suspend failed!"; }

  { LOG(LogInfo) << "[RG353X] WAKEUP!"; }
