...
std::string Upgrade::GetDomainName()
{
  ...
  // If target has been set to patron, we set it as unexisting, to avoid the upgrade on patron if the key is not valid
  // But also to avoid an upgrade on stable
  if(target == "patron")
    target = "unexisting";
...
std::string Upgrade::GetRemoteVersion()
{
  ...
  for(int i = 3; --i >= 0; )
    if (request.Execute(url, version))
    {
      ...
      if (returnCode == 200) break;  // Exit switch
