...
void PadToKeyboardManager::StartMapping()
{
  ...
  {
    { LOG(LogInfo) << "[Pad2Keyboard] No pad2keyb configuration."; }
    return;
  }
  ...
  {
    { LOG(LogError) << "[Pad2Keyboard] Cannot load pad configurations."; }
    return;
  }
  ...
  if (!mPadReader.Ready())
  {
    { LOG(LogError) << "[Pad2Keyboard] Cannot access pad."; }
    return;
  }
  ...
  {
    { LOG(LogError) << "[Pad2Keyboard] Cannot create virtual keyboard."; }
    return;
  }
