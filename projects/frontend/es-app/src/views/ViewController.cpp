...
void ViewController::LaunchCheck(FileData* game, const Vector3f& cameraTarget, bool forceLaunch)
{
  ...
  if (mSystemManager.Emulators().GetGameEmulator(*game).IsLibretro()
  && RecalboxConf::Instance().GetGlobalShowSaveStateBeforeRun()
  && !GameFilesUtils::GetGameSaveStateFiles(*game).empty()
  && !mGameLinkedData.ConfigurableSaveState().IsConfigured() )
  {
