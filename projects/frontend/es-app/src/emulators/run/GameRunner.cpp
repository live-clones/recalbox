...
bool GameRunner::RunGame(FileData& game, const EmulatorData& emulator, const GameLinkedData& data)
{
  ...
  && data.Patch().PatchPath().Directory() != game.FilePath().Directory())
  {
      if (data.Patch().PatchPath().Extension() == ".ips")
      {
        command.append(" -ips").append(data.Patch().PatchPath().ToString());
      }
      else if (data.Patch().PatchPath().Extension() == ".bps")
      {
        command.append(" -bps").append(data.Patch().PatchPath().ToString());
      }
      else if (data.Patch().PatchPath().Extension() == ".ubs")
      {
        command.append(" -ubs").append(data.Patch().PatchPath().ToString());
      }
  }
