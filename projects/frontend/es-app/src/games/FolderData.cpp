...
int FolderData::getItemsRecursively(FileData::List& to, IFilter* filter, bool includefolders, bool includeadult) const
{
  ...
  for (FileData* fd : mChildren)
  {
    ...
    {
      if (filter->ApplyFilter(*fd) && (includeadult || !fd->Metadata().Adult()) && (!RecalboxConf::Instance().GetLatestRevOnly() ||
        fd->Metadata().LatestRev()))
      {
        to.push_back(fd);
        gameCount++;
      }
...
int FolderData::getItemsRecursively(FileData::List& to, Filter includes, bool includefolders, bool includeadult) const
{
  ...
  for (FileData* fd : mChildren)
  {
    ...
    else if (fd->IsGame())
    {
      ...
      if ((current & includes) != 0 && (includeadult || !fd->Metadata().Adult()) && (!RecalboxConf::Instance().GetLatestRevOnly() ||
        fd->Metadata().LatestRev()))
      {
        to.push_back(fd);
        gameCount++;
    ...
    if (fd->IsFolder())
    {
      ...
      if (folder->mChildren.size() == 1)
      {
        ...
        if (item->IsGame())
        {
          ...
          if (current == 0) current = Filter::Normal;
          if ((current & includes) != 0 && (includeadult || !item->Metadata().Adult()) && (!RecalboxConf::Instance().GetLatestRevOnly() ||
            fd->Metadata().LatestRev()))
            isolatedFile = item;
        }
      }
