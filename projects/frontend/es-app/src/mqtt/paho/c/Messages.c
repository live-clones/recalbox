...
const char* Messages_get(int index, enum LOG_LEVELS log_level)
{
	...
	if (log_level == TRACE_PROTOCOL)
		msg = (index >= 0 && index < (int)ARRAY_SIZE(protocol_message_list)) ? protocol_message_list[index] : NULL;
	else
		msg = (index >= 0 && index < (int)ARRAY_SIZE(trace_message_list)) ? trace_message_list[index] : NULL;
	return msg;
}
