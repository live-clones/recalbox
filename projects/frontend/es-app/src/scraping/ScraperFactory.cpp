...
IScraperEngine* ScraperFactory::Get(ScraperType type)
{
  // Ensure valid type
  switch(type)
  {
  ...
  switch(type)
  {
    case ScraperType::ScreenScraper: result = new ScreenScraperEngineImplementation(); break;
    case ScraperType::Recalbox     : result = new RecalboxEngineImplementation(); break;
...
IScraperEngine* ScraperFactory::GetScraper(ScraperType scraper)
{
  // Get
  IScraperEngine* engine = Get(scraper);
  // (re)Initialize
  engine->Initialize();
