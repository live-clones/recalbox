...
GuiSaveStates::GuiSaveStates(WindowManager& window, SystemManager& systemManager, FileData& game)
  : Gui(window),
...
bool GuiSaveStates::ProcessInput(const class InputCompactEvent & event)
{
  ...
  else if (event.ValidPressed() && mIsLibretro && mCurrentState.GetPath().Extension() != ".auto")
  {
    launch();
    return true;
  }
  ...
  help.Set(HelpType::UpDown, _("CHOOSE"))
    ...
    if (!mList->isEmpty())
    {
      if (mIsLibretro && mCurrentState.GetPath().Extension() != ".auto")
        help.Set(Help::Valid(), _("LAUNCH GAME FROM STATE SLOT"));
...
void GuiSaveStates::launch()
{
  if (mList->size() != 0)
  {
    ...
    ViewController::Instance().Launch(&mGame, GameLinkedData(slot), target);
  }
}
