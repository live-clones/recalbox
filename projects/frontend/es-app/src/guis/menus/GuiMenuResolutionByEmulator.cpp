...
std::vector<GuiMenuBase::ListEntry<std::string>> GuiMenuResolutionByEmulator::GetResolutionEntries(SystemData& system)
{
  ...
  result.push_back({ _("UNDEFINED (USE GLOBAL)"), "", !RecalboxConf::Instance().IsDefinedSystemVideoMode(system) });
  result.push_back({ _("SYSTEM DEFAULT"), "default", RecalboxConf::Instance().GetSystemVideoMode(system) == "default" });
