...
std::vector<GuiMenuBase::ListEntry<std::string>> GuiMenuResolutionSettings::GetGlobalResolutionEntries()
{
  ...
  result.push_back({ _("SYSTEM DEFAULT"), "default", !RecalboxConf::Instance().IsDefinedGlobalVideoMode() || RecalboxConf::Instance().GetGlobalVideoMode() == "default" });
...
{
  std::vector<GuiMenuBase::ListEntry<std::string>> result;
  result.push_back({ _("UNDEFINED (USE GLOBAL)"), "", !RecalboxConf::Instance().IsDefinedEmulationstationVideoMode() });
  result.push_back({ _("SYSTEM DEFAULT"), "default", RecalboxConf::Instance().GetEmulationstationVideoMode() == "default" });
  for(const Renderer::Resolution& resolution : Renderer::GetFilteredResolutionList())
  {
    std::string reso = resolution.ToString();
