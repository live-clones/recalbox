...
const std::string MenuMessages::UPDATE_CHANGELOG_HELP_MSG = _("Shows the current available update changelog.");
...
const std::string MenuMessages::GAMELISTOPTION_JUMP_LETTER = _("Select a letter and the listing will go directly on the first game starting with this letter.");
const std::string MenuMessages::GAMELISTOPTION_SORT_GAMES = _("Select the way the game list is sortered (alphabetically, by notation...).");
const std::string MenuMessages::GAMELISTOPTION_FAVORITES_ONLY = _("Switch between seing or not only the favorites games. To add a game in the favorite list, select the game and toggle its state using 'Y'.");
const std::string MenuMessages::GAMELISTOPTION_SHOW_HIDDEN = _("Switch between seing or not the hidden games. To hide a game, edit its data and select 'Hide'.");
const std::string MenuMessages::GAMELISTOPTION_EDIT_METADATA = _("This option display a menu which allows to change game data and many others options.");