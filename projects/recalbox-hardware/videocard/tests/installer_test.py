...
def test_enable_nvidia_drm_modeset(mocker):
    mocker.patch("builtins.open", mocker.mock_open())

    Nvidia.installer.enable_nvidia_drm_modeset()

    open.assert_called_once_with("/etc/modprobe.d/nvidia-drm.conf", "w", encoding="utf8")
    open().write.assert_called_once_with("options nvidia-drm modeset=1")
