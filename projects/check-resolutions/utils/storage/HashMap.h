...
public:
	...
	private:
		void goto_next_element()
		{
			...
			do {
				_bucket++;
			} while (_map->_states[_bucket] != State::FILLED);
		}
	...
	{
		size_t bucket = 0;
		while (_states && _states[bucket] != State::FILLED) {
			++bucket;
		}
	...
	{
		size_t required_buckets = num_elems + num_elems / 5 + 2;
		...
		_states[num_buckets] = State::FILLED;
