...
if [[ "${mode:0:3}" == "r64" ]]; then
# Rock64 Overclocking

cpu_gov="mrfixedfreq12"
gpu_gov="mrfixedfreq5"
if [[ "$mode" == "r64-low" ]]; then
cpu_gov="mrfixedfreq13"
gpu_gov="mrfixedfreq6"
elif [[ "$mode" == "r64-high" ]]; then
cpu_gov="mrfixedfreq14"
gpu_gov="mrfixedfreq7"
elif [[ "$mode" == "r64-extrem" ]]; then
cpu_gov="mrfixedfreq15"
gpu_gov="mrfixedfreq8"
fi

cmdtxt=' /bin/sh -c '"'"' echo '"${cpu_gov}"' > /sys/devices/system/cpu/cpufreq/policy0/scaling_governor'"'"$' \n /bin/sh -c '"'"' echo '"${gpu_gov}"' > /sys/class/devfreq/ff300000.gpu/governor'"'"' '
echo "$cmdtxt" > /etc/init.d/S100overclock

chmod +x /etc/init.d/S100overclock
...
if [[ "$command" == "toggleRockProFan" ]]; then
    recallog "Toggling the RockPro Fan"
    /etc/init.d/S99rockprofan restart
    exit 0
fi

if [[ "$command" == "restartBT" ]]; then
    recallog "Restarting bluetooth services"
