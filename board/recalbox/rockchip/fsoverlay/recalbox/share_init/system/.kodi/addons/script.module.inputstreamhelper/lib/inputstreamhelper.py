...
class Helper(object):
    def __init__(self, protocol, drm=None):
        ...
        self._log('Testing Arch: {0}'.format(struct.calcsize("P") * 8))
    ...
    def _install_widevine_arm(self):
        ...
        if dialog.yesno(LANGUAGE(30001),
            ...
            if downloaded:
                ...
                self._log('bin path: {0}'.format(bin_path))
    ...
    def _install_cdm(self):
        ...
        self._log('Using cdm path={0}'.format(self._addon_cdm_path()))
        for cdm_file in os.listdir(self._addon_cdm_path()):
            if cdm_file.endswith(config.CDM_EXTENSIONS):
                ...
                self._log('Found Paths: cdm_path_addon={0}, cdm_path_inputstream={1}'.format(cdm_path_addon, cdm_path_inputstream))
