...
case "${RECALBOX_TARGET}" in
    RPI0|RPI1|RPI2|RPI3)
	# recalbox squashfs
	cp "${BINARIES_DIR}/rootfs.squashfs" "${RECALBOX_BINARIES_DIR}/recalbox.squashfs.upgrade" || exit 1
	ls -l "${RECALBOX_BINARIES_DIR}/recalbox.squashfs.upgrade" | cut -d " " -f 5 > "${RECALBOX_BINARIES_DIR}/recalbox.squashfs.size"
	xz --threads=0 "${RECALBOX_BINARIES_DIR}/recalbox.squashfs.upgrade"
