...
#file wich contains path to system videos (in /recalbox/share/system to be easily editable by user)
...
#~ getStateData()
#~ {
	#~ #fill the useful variables (thanks Mika for the regular expression ;))
	#~ TFTAction="$(sed -n 's/^Action=\([^\r]\+\)\r\?$/\1/p' /tmp/es_state.inf)"
	#~ # action in : systembrowsing, gamelistbrowsing, rungame, rundemo, endgame, enddemo
	...
	#~ TFTESState="$(sed -n 's/^State=\([^\r]\+\)\r\?$/\1/p' /tmp/es_state.inf)"
#~ }
...
do_start() {
    ...
    # test if ${DEV} is present to be able to display stuff
	if [ -c "${DEV}" ]; then
		...
		#echo "${FFMPEG}" -re -i "${LogoFolder}"/"${tftResolution}"/"${DefaultStartLogo}" -pix_fmt rgb565le -f fbdev "${DEV}" 2>/dev/null >/dev/null
		
		"${FFMPEG}" -re -i "${LogoFolder}"/"${tftResolution}"/"${DefaultStartLogo}" -pix_fmt rgb565le -f fbdev "${DEV}" 2>/dev/null >/dev/null
			...
			if [[ "${ATIME}" != "${LTIME}" ]]  || [[ "${ASIZE}" != "${LSIZE}" ]] || [[ "${ATIME2}" != "${LTIME2}" ]] ; then
				...
				#case "${eventES}" in
					...
					systembrowsing)     
						...
						dd if=/dev/zero of="${DEV}" 2>/dev/null >/dev/null   #clear before draw
						usleep "${tempoShort}"
						"${FFMPEG}" -re -stream_loop -1 -i "${LogoFolder}"/"${tftResolution}"/"${LogoFile}" -c:v rawvideo -pix_fmt rgb565le -f fbdev "${DEV}" 2>/dev/null >/dev/null &
						;;
					gamelistbrowsing|endgame|enddemo)
						...
						if [[ "${TFTGame}" != "${previousTFTGameBrowse}" ]]; then
							...
							previousTFTESSystem=""
							if [[ "$(pidof ffmpeg)" ]] ; then
								#kill -9 "$(pidof ffmpeg)" 2>/dev/null >/dev/null
								killall ffmpeg 2>/dev/null >/dev/null
								dd if=/dev/zero of="${DEV}" 2>/dev/null >/dev/null   #clear before draw
								usleep "${tempoShort}"
							fi
							...
							if [[ -z "${TFTVideoPath}" ]]; then 
								FRAMEBUFFER="${DEV}" fbv --stretch --delay 1 "${TFTGameImage}" 2>/dev/null >/dev/null &
							else
								FRAMEBUFFER="${DEV}" fbv --stretch --delay 1 "${TFTGameImage}" 2>/dev/null >/dev/null &
								usleep "${tempoLong}"
								"${FFMPEG}" -re -stream_loop -1 -i "${TFTVideoPath}" -s "${tftFullResolution}" -c:v rawvideo -pix_fmt rgb565le -f fbdev "${DEV}" 2>/dev/null >/dev/null &
							fi
						fi
						;;
					rungame|rundemo)
						...
						if [[ "${TFTGame}" != "${previousTFTGameRun}" ]]; then
							...
							dd if=/dev/zero of="${DEV}" 2>/dev/null >/dev/null   #clear before draw
							usleep "${tempoShort}"
							"${FFMPEG}" -re -i "${LogoFolder}"/"${tftResolution}"/"${LogoFile}" -c:v rawvideo -pix_fmt rgb565le -f fbdev "${DEV}" 2>/dev/null >/dev/null
							dd if=/dev/zero of="${DEV}" 2>/dev/null >/dev/null   #clear before draw
							usleep "${tempoShort}"
							FRAMEBUFFER="${DEV}" fbv --stretch --delay 1 "${TFTGameImage}" 2>/dev/null >/dev/null
						fi
