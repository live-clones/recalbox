...
NVIDIA_DRIVER_390_RECALBOX_VERSION = 390.143
...
# libGL.so.$(NVIDIA_DRIVER_390_RECALBOX_VERSION) is the legacy libGL.so library; it
...
define NVIDIA_DRIVER_390_RECALBOX_INSTALL_TARGET_CMDS
	...
	$(INSTALL) -D -m 0644 $(@D)/10_nvidia.json $(TARGET_DIR)/usr/share/glvnd/egl_vendor.d/10_nvidia.json.$(NVIDIA_DRIVER_390_RECALBOX_VERSION)
	$(INSTALL) -D -m 0644 $(@D)/10_nvidia_wayland.json $(TARGET_DIR)/usr/share/glvnd/egl_vendor.d/10_nvidia_wayland.json.$(NVIDIA_DRIVER_390_RECALBOX_VERSION)
	$(INSTALL) -D -m 0644 $(@D)/nvidia-drm-outputclass.conf $(TARGET_DIR)/usr/share/X11/xorg.conf.d/10-nvidia-drm.conf.$(NVIDIA_DRIVER_390_RECALBOX_VERSION)
	$(INSTALL) -D -m 0755 $(@D)/nvidia-settings $(TARGET_DIR)/usr/bin/nvidia-settings.$(NVIDIA_DRIVER_390_RECALBOX_VERSION)
	$(INSTALL) -D -m 0755 $(@D)/nvidia-smi $(TARGET_DIR)/usr/bin/nvidia-smi.$(NVIDIA_DRIVER_390_RECALBOX_VERSION)
endef
