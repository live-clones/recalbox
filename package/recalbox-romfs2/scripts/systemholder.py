...
class SystemHolder:
    ...
    class Core:
        ...
        def __init__(self, package: str, priority: int, emulator: str, core: str, extensions: str, netplay: bool, compatibility: str, speed: str):
            ...
            if compatibility not in self.__CompatibilityValues.keys(): raise TypeError("Invalid system type! {}".format(compatibility))
    ...
    def __deserialize(self):
        ...
        self.__screenscraperId = int(self.__get(desc, "system", "screenscraper.id", "", True))
        self.__command = self.__get(desc, "system", "command", "python /usr/lib/python2.7/site-packages/configgen/emulatorlauncher.pyc %CONTROLLERSCONFIG% -system %SYSTEM% -rom %ROM% -emulator %EMULATOR% -core %CORE% -ratio %RATIO% %NETPLAY%", False)
        ...
        for i in range(0, 10):
            ...
            if desc.hasSection(coreSection) or desc.hasSection(coreSectionArch):
                ...
                core = SystemHolder.Core(
                    ...
                    compatibility=self.__get(desc, coreSection, "compatibility", "", False),
                    speed=self.__get(desc, coreSection, "speed", "", False),
                )
                # Package defined?
