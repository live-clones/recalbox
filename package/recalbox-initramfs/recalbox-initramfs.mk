...
RECALBOX_INITRAMFS_BINARIES_SUBDIR=$(BINARIES_DIR)/initramfs
...
RECALBOX_INITRAMFS_MAKE_OPTS = \
	CC="$(TARGET_CC)" \
	ARCH=$(KERNEL_ARCH) \
	PREFIX="$(RECALBOX_INITRAMFS_BINARIES_SUBDIR)" \
	EXTRA_LDFLAGS="$(RECALBOX_INITRAMFS_LDFLAGS)" \
	CROSS_COMPILE="$(TARGET_CROSS)" \
	CONFIG_PREFIX="$(RECALBOX_INITRAMFS_BINARIES_SUBDIR)" \
	SKIP_STRIP=y
...
define RECALBOX_INITRAMFS_INSTALL_TARGET_CMDS
	mkdir -p $(RECALBOX_INITRAMFS_BINARIES_SUBDIR)/{new_root,sys,proc}
	cp $(RECALBOX_INITRAMFS_PKGDIR)/init $(RECALBOX_INITRAMFS_BINARIES_SUBDIR)/init
	$(RECALBOX_INITRAMFS_MAKE_ENV) $(MAKE) $(RECALBOX_INITRAMFS_MAKE_OPTS) -C $(@D) install
	(cd $(RECALBOX_INITRAMFS_BINARIES_SUBDIR) && find . | cpio -H newc -o > $(BINARIES_DIR)/initrd)
...
define RECALBOX_INITRAMFS_INSTALL_TARGET_CMDS
	...
	cp $(RECALBOX_INITRAMFS_PKGDIR)/init $(RECALBOX_INITRAMFS_BINARIES_SUBDIR)/init
	$(RECALBOX_INITRAMFS_MAKE_ENV) $(MAKE) $(RECALBOX_INITRAMFS_MAKE_OPTS) -C $(@D) install
	(cd $(RECALBOX_INITRAMFS_BINARIES_SUBDIR) && find . | cpio -H newc -o | gzip -9 > $(BINARIES_DIR)/initrd.gz)
endef
