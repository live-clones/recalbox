...
HHPC_LICENSE = BSD-3c
HHPC_DEPENDENCIES = xserver_xorg-server host-pkgconf

define HHPC_BUILD_CMDS
	$(TARGET_MAKE_ENV) $(MAKE) $(TARGET_CONFIGURE_OPTS) \
		CXX="$(TARGET_CXX)" CC="$(TARGET_CC)" -C $(@D) DESTDIR="$(TARGET_DIR)" PREFIX="/usr"
endef

define HHPC_INSTALL_TARGET_CMDS
	$(INSTALL) -D $(@D)/hhpc $(TARGET_DIR)/usr/bin/hhpc
endef
